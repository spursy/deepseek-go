...
func MultiChat() {
	...
	response1, _, _, err := client.CreateChatCompletion(ctx, &deepseek.ChatCompletionRequest{
	...
	response2, _, _, err := client.CreateChatCompletion(ctx, &deepseek.ChatCompletionRequest{
		Model:    deepseek.DeepSeekChat,
		Messages: messages,
	})
	if err != nil {
